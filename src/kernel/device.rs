...
extern "C" {
    ...
    fn loss_input_transform_to_features_float(self_output: *mut f32, opponent_output: *mut f32, combined_input: *const f32, nlen: size_t, batch_size: size_t) -> c_void;
    fn loss_input_transform_to_features_double(self_output: *mut f64, opponent_output: *mut f64, combined_input: *const f64, nlen: size_t, batch_size: size_t) -> c_void;
}
...
/// that converts the output of the computation of the loss of the LinearLayer to the loss of the FeatureTransformLayer.
...
/// Creates an instance of an object representing a list of arguments to be passed to the Cuda kernel
/// that transforms the output of the loss of a FeatureTransformLayer.
impl<T> LossInputTransformToFeaturesArgs<T> where T: DataTypeInfo {
    /// Create a LossInputTransformToFeaturesArgs instance
    /// # Arguments
    /// * `self_output` - active side loss output
    /// * `opponent_output` - non-active side loss output
    /// * `combined_input` - combined_input
    /// * `out_len` - Number of scalar values in forward output
    /// * `batch_len` - batch_count
    pub fn new(self_output:CudaPtr<T>,opponent_output:CudaPtr<T>,combined_input:CudaPtr<T>,out_len:usize,batch_len:usize)
